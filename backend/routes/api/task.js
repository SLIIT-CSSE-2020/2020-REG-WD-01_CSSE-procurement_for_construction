...
router.post("/task", (req, res) => {
  var taskObj = new Task({
    ...
    descripition: req.body.formData.description,
  });
...
router.post("/task-delete", (req, res) => {
  ...
  TaskList.deleteOne({ taskId: req.body.userId }).exec(function (err, tasks) {
    if (err) {
      return console.error(err);
    } else {
      Task.deleteOne({ _id: req.body.userId }).exec(function (err1, tasks1) {
        if (err1) {
          console.error(err1);
        } else {
          return res.status(200).json({
